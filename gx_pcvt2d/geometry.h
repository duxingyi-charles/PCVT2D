...
namespace Geex {
	...
	template <class T> inline vec2g<T> triangle_centroid(
		...
		T a, T b, T c // a, b, c are density at p, q, r
			...
			T abc = a+b+c ; 
			T area = triangle_area(p, q, r) ;
			//T V = area/T(3)*abc ;
			return (area/T(12))*((a+abc)*p+(b+abc)*q+(c+abc)*r) ;
	}
	///
	...
	template <class T> inline vec2g<T> polygon_centroid(std::vector<vec2g<T>>& P) {
		vec2g<T> mg(0, 0) ;
		T        a = 0 ;
		...
		for(int i=1; i<P.size()-1; ++i) {
			vec2 pi = triangle_centroid(P[0], P[i], P[i+1]) ;
			double ai = triangle_area(P[0], P[i], P[i+1]) ;
			mg += ai*pi ;
			a += ai ;
		}
		return mg/a ;
