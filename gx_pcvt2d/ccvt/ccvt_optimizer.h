...
namespace ccvt {
  ...
  private:
    ...
    private:
      ...
      NearestNeighborResult nearest_neighbor(Node* node, const Point& point, const int depth, double& minDistance) {
        if (node->disabled) { // dmyan: static_cast is required here
          return NearestNeighborResult(static_cast<Node*>(NULL), static_cast<Entry*>(NULL));
        }
  ...
  public:
    ...
    void initialize(std::list<Site>& sites, std::list<Point>& points, const Metric& metric) {
      clear();
