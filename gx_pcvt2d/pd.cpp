...
static void usage()
{
    std::cerr << "usage: pd [min num points] [max num points]" << endl;
    exit(1);
}

int genDisk(int minPoints, int maxPoints, vector<Point2>& result)
{
...
int genDisk(double radius, std::vector<Point2>& points) {
	INIT_RANDOM_NUMBER_GENERATOR() ;
	int nThrows = 2000*sqrt(3.)/(6*radius*radius) ;
	std::cerr << "number of throws: " << nThrows << std::endl ;
	std::cerr << "sampling radius: " << radius << std::endl ;
	throwDarts(nThrows, radius, &points) ;
